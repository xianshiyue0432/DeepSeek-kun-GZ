...
> 把 Kun 的本地智能体能力带进桌面窗口：**Code** 处理项目、**写作**打磨文档、**连接手机**接入 IM 与定时任务——聊天、计划、目标、代码审查、Skill/MCP 和更新，都在一个图形化工作台里完成。
...
- **Token ROI 优化**：Kun 会稳定 prompt 前缀、跟踪 DeepSeek 原生缓存命中、按需压缩上下文和工具输出，并可用 MCP search 渐进发现工具，减少“把无关 token 发给模型”的浪费。
...
工具输出、无效历史或已经可以被缓存复用的前缀上。

Kun 集成了两套已被验证的设计：

- **借鉴自 [CodeWhale](https://github.com/Hmbown/CodeWhale) 的本地 HTTP serve 边界**：`/v1/threads`、`/v1/threads/{id}/turns`、`/v1/threads/{id}/events`（SSE）、`/v1/approvals/{id}`、`/v1/usage`、`/v1/workspace/status`、`/v1/runtime/info`、`/health`。这一形态让 GUI 与 runtime 完全解耦：runtime 升级、调试、替换都不需要 GUI 跟改。
...
> 致谢：感谢 [Hmbown/CodeWhale](https://github.com/Hmbown/CodeWhale)
> 团队和 Reasonix 团队为这两个关键设计提供的可运行参考。Kun
> 的几乎全部性能特征——cache hit 率、token replay、断线重连、
> 审批中断——都可以追溯到这两个项目。具体设计取舍与借鉴映射
...
npm run release:all     # 在 macOS 上构建 macOS/Windows/Linux 并创建 GitHub release
```
...
- **[CodeWhale](https://github.com/Hmbown/CodeWhale)** —— 本地 HTTP serve 边界。`/v1/threads`、`/v1/threads/{id}/turns`、`/v1/threads/{id}/events` (SSE)、`/v1/approvals/{id}`、`/v1/usage`、`/v1/workspace/status`、`/v1/runtime/info`、`/health` 这套 URL shape 直接借鉴自 CodeWhale 的 `serve --http`。GUI 与 runtime 之间只留一个标准化、跨语言、跨进程的边界，让我们能像调试网络协议一样调试 agent 行为。
